...
-- 수정된 RLS 정책들 (UUID 타입 캐스팅)
CREATE POLICY "Fathers can view all advices" ON advices
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM users 
            WHERE id = advices.author_id::uuid AND user_type = 'father'
        )
    );
...
CREATE POLICY "Fathers can insert advices" ON advices
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM users 
            WHERE id = author_id::uuid AND user_type = 'father'
        )
    );
...
DECLARE
    father_id UUID;
BEGIN
    SELECT id INTO father_id FROM users WHERE email = 'father@example.com';
    
    -- 샘플 조언 데이터 (미디어 포함)
    INSERT INTO advices (author_id, category, target_age, content, media_url, media_type) VALUES
