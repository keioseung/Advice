-- =====================================================
-- Advice App - Final Database Fix
-- 기존 테이블 구조에 맞춰서 안전하게 수정
-- =====================================================
...
-- 2단계: Users 테이블에 age 컬럼 추가 (가장 안전한 방법)
ALTER TABLE users ADD COLUMN IF NOT EXISTS age INTEGER;

-- 3단계: age 컬럼에 제약 조건 추가 (오류 무시하고 시도)
DO $$
BEGIN
    BEGIN
...
DROP POLICY IF EXISTS "Children can read all advices" ON advices;
...
UNION ALL
SELECT 'Current Advices' as info, COUNT(*) as count FROM advices;

-- 15단계: 기존 데이터가 없는 경우에만 샘플 데이터 추가
DO $$
DECLARE
    user_count INTEGER;
...
BEGIN
    ...
    SELECT COUNT(*) INTO advice_count FROM advices;
    
    -- 사용자 데이터가 없는 경우에만 추가
    IF user_count = 0 THEN
        -- 아버지 계정 샘플 (모든 필수 컬럼 포함)
        INSERT INTO users (id, user_id, password_hash, user_type, name, father_id, age) 
        VALUES 
            ('father1', 'father1', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj4J/HS.iK2', 'father', '김철수', NULL, NULL);
        
        -- 자녀 계정 샘플 (모든 필수 컬럼 포함)
        INSERT INTO users (id, user_id, password_hash, user_type, name, father_id, age) 
        VALUES 
            ('child1', 'child1', '$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj4J/HS.iK2', 'child', '김영희', 'father1', 25);
    END IF;
...
-- 17단계: 최종 데이터 확인
SELECT 'Final Users' as info, COUNT(*) as count FROM users
UNION ALL
SELECT 'Final Advices' as info, COUNT(*) as count FROM advices;
