-- supabase_schema_with_storage.sql 파일을 Supabase SQL Editor에서 실행
...
-- Policy for public read access to advice-media bucket
-- This allows anyone to read files from the advice-media bucket
-- Go to Supabase Dashboard > Storage > advice-media bucket > Policies
-- Add policy: "Public read access" with condition: true for SELECT

-- Policy for authenticated users to upload to advice-media bucket
-- This allows authenticated users to upload files to the advice-media bucket
-- Go to Supabase Dashboard > Storage > advice-media bucket > Policies
-- Add policy: "Authenticated users can upload" with condition: auth.role() = 'authenticated' for INSERT

-- Policy for file owners to delete from advice-media bucket
-- This allows file owners to delete their own files
-- Go to Supabase Dashboard > Storage > advice-media bucket > Policies
-- Add policy: "File owners can delete" with condition: auth.uid()::text = (storage.foldername(name))[1] for DELETE 