-- 애비의 조언 앱 - 미디어 기능 포함 스키마 (public 스키마 사용)
...
FROM users u
LEFT JOIN advices a ON u.user_id = a.author_id
...
DECLARE
    father_id UUID;
BEGIN
    SELECT user_id INTO father_id FROM users WHERE email = 'father@example.com';
