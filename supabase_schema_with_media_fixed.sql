...
-- 기존 뷰 삭제 (에러 방지)
DROP VIEW IF EXISTS advice_app.advice_stats;
...
-- 트리거 생성
DROP TRIGGER IF EXISTS update_users_updated_at ON advice_app.users;
CREATE TRIGGER update_users_updated_at 
    BEFORE UPDATE ON advice_app.users 
    FOR EACH ROW EXECUTE FUNCTION advice_app.update_updated_at_column();

DROP TRIGGER IF EXISTS update_advices_updated_at ON advice_app.advices;
...
-- 기존 RLS 정책 삭제
DROP POLICY IF EXISTS "Users can view their own profile" ON advice_app.users;
DROP POLICY IF EXISTS "Users can update their own profile" ON advice_app.users;
DROP POLICY IF EXISTS "Fathers can view all advices" ON advice_app.advices;
DROP POLICY IF EXISTS "Children can view advices from their father" ON advice_app.advices;
DROP POLICY IF EXISTS "Fathers can insert advices" ON advice_app.advices;
DROP POLICY IF EXISTS "Fathers can update their advices" ON advice_app.advices;
DROP POLICY IF EXISTS "Fathers can delete their advices" ON advice_app.advices;
...
DELETE FROM advice_app.advices;
DELETE FROM advice_app.users;
